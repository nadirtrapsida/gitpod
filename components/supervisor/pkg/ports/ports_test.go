	...
	tests := []struct {
		...
		},
		{
			...
			ExpectedUpdates: UpdateExpectation{
				...
				{Updated: []*api.PortsStatus{
					{LocalPort: 4040, GlobalPort: 60000, Served: true, Exposed: &api.PortsStatus_ExposedPortInfo{Public: true, Url: "4040-foobar", OnExposed: api.PortsStatus_ExposedPortInfo_open_browser}},
				}},
			},
		},
		{
		...
		t.Run(test.Desc, func(t *testing.T) {
			...
			go func() {
				...
				for _, c := range test.Changes {
					if c.Config != nil {
						change := &Configs{}
						change.workspaceConfigs = configParser.parseWorkspaceConfigs(c.Config.workspace)
						portConfigs, rangeConfigs := configParser.parseInstanceConfigs(c.Config.instance)
